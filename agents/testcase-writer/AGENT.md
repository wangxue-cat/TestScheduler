...
| Skill | 描述 | 来源 |
|-------|------|------|
| `api-testcase-writer` | 核心：7步生成流水线（解析需求ID→重复检查→加载材料→识别渠道→应用编写规则→生成Excel→输出摘要） | 自有 |
| `review-testcases` | 评审用例覆盖度、重复检测、遗漏场景识别 | 自有 |
| `upload-testcases` | 上传用例 Excel 到 QOA 平台并关联 Story | 自有 |
