...
独立的多Agent测试调度平台，覆盖从需求分析到知识沉淀的完整测试生命周期。与 ClaudeMind 并行运行，共享 testmind 底层技能引擎。

## Agent 隔离规则

**本项目禁止回退到 ClaudeMind 的全局 Agent**。TestScheduler 的所有操作必须使用以下方式之一：

1. **通过 testmind-facade 门面调用 testmind Skill**：所有 `Skill(testmind:xxx)` 必须通过 [testmind-facade](agents/testmind-facade/SKILL.md) 门面层，自动加载经验文件
2. **使用 TestScheduler 本地 Agent**：`d:\TestScheduler\agents\` 下的 9 个 Agent，产出路径为 `d:\TestScheduler\memory\`
3. **主会话直接执行**：按本文件路由规则逐步调度

以下全局 Agent 仅限 ClaudeMind 项目使用，TestScheduler **禁止调用**：
- `collect-requirement-materials` → 改用 `Skill(testmind:request-manage)` + `Skill(testmind:story-manage)` + `Skill(testmind:get-request-content)` + 直接 Write
- `api-auto-testing`、`executor`、`info-fetcher` 等 → 改用 TestScheduler 对应的本地 Agent 或 Skill

> 违反此规则会导致数据写入 `D:\ClaudeMind\memory\` 而非 `d:\TestScheduler\memory\`，造成项目隔离失效。

## 文件读取隔离规则

> 🚫 **强制规则：禁止从 `D:\ClaudeMind\` 目录读取任何文件。**

TestScheduler 是独立项目，建设完成后 `D:\ClaudeMind` 将被删除。如果现在从 ClaudeMind 读取文件，将无法发现 TestScheduler 自身的建设欠缺。

### 禁止行为

- ❌ 读取 `D:\ClaudeMind\memory\` 下的任何文件（包括 api_channels、api_channels_rules、db_info 等共享资源）
- ❌ 读取 `D:\ClaudeMind\CLAUDE.md` 或任何 ClaudeMind 配置
- ❌ 使用 ClaudeMind 的 skill、agent、knowledge 文件
- ❌ 读取 `D:\ClaudeMind\` 路径下的任何内容

### 替代方案

| 需要的数据 | TestScheduler 获取方式 |
|-----------|----------------------|
| 渠道接口定义 | 从自动化平台查询（`testmind:auto-interface-list`），`api_channels/` 已废弃不再使用 |
...
如果 TestScheduler 本地缺少某个文件导致无法完成任务，**不要去 ClaudeMind 找**。直接告知用户文件缺失，这正是项目的建设欠缺所在。
...
| **Test Case Writer** | `testcase-writer` | 用例编写：生成Excel→评审→上传QOA | 移植自 ClaudeMind |
...
> 🚫 **强制规则：所有 `Skill(testmind:xxx)` 调用必须通过 `Skill(testmind-facade)` 门面，严禁直接调用。**
>
> **唯一正确调用链**：`Skill(testmind-facade) → 门面内部路由 → Skill(testmind:xxx)`，禁止主会话或 Agent 跳过门面直接调 `Skill(testmind:xxx)` 或 Bash 脚本。
>
> **门面执行协议**（详见 [SKILL.md](agents/testmind-facade/SKILL.md)）：
> 1. LOAD → 加载 `experience/{skill-name}.md`
> 2. RESOLVE → 查 [skill-registry.md](agents/testmind-facade/refs/skill-registry.md) 确定路由（委托本地 wrapper 或直调 testmind）
> 3. EXECUTE → 带着经验上下文执行
> 4. OBSERVE → 检查新发现
> 5. WRITE-BACK → 追加到经验文件的 EVOLUTION_MARKER
>
> **经验文件**：首次调用时自动从 [refs/experience-template.md](agents/testmind-facade/refs/experience-template.md) 创建 stub。
...
> ⚠️ **接口定义不再使用本地 JSON 文件**。所有渠道接口定义统一在 QOA 自动化平台维护，通过 `testmind:auto-interface-list` 查询。`memory/api_channels/` 目录已废弃，TestScheduler 不读取也不写入。

TestScheduler 以**只读**方式使用共享资源；写入操作（新增接口、更新规则）通过 Platform Manager Agent 完成。
