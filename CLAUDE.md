...
Step 3: Agent(test-mapper)
  提示: "使用 parse-case-steps → verify-executability → match-platform-interface → generate-execution-plan 映射需求 {id} 的用例"
  输入: memory/testcases/{id}_testcases.xlsx
  产出: memory/execution_plans/{id}_plan.json + memory/execution_plans/{id}_executability_report.md
  检查: plan.json 存在且 cases 非空 → 进入 Step 4
  有 block: 🔴 告知用户必须先解决阻塞项再重试
...
> 🚫 **强制规则：所有 testmind skill 的执行必须通过 `Skill(testmind:xxx)` 调用，禁止直接 subprocess/脚本调用。**
>
> QOA 平台通过 `testmind:schedule` 命令搜集并上报 skill 执行次数。直接 subprocess 调 `execute_sql.py`、`bug_manage.py` 等底层脚本会绕过追踪。

### 正确执行方式

| 场景 | ✅ 正确 | ❌ 禁止 |
|------|--------|--------|
| SQL 执行 | Phase A: `python run.py --resolve-only` → Phase B: `Skill(testmind:sql-execute, args)` | `python execute_sql.py --env ... --sql ...` |
| 小工具调用 | Phase A: wrapper 解析 → Phase B: `Skill(testmind:common-tool-execute, args)` | 裸 `urllib.request` / curl HTTP 调用 |
| Bug 管理 | `Skill(testmind:bug-manage, args)` | `python bug_manage.py create ...` |
| Story 管理 | `Skill(testmind:story-manage, args)` | `python story_manage.py transition ...` |
| 查询日志 | `Skill(testmind:query-log, args)` | 直接 HTTP 调日志 API |
| 灵犀调度 | `Skill(testmind:lingxi-scheduler, args)` | `python qoa_api.py scheduler ...` |
| 其他所有 testmind skill | `Skill(testmind:xxx, args)` | 任何 subprocess / 直接脚本调用 |

### wrapper 的角色

Wrapper（如 `sql-execute/run.py`、`execute-tool/`）仅负责 **Phase A: 本地解析**（环境路由、库匹配、参数组装），**执行必须交给 `Skill(testmind:xxx)`**。Wrapper 禁止在内部 subprocess 调用底层脚本。
