...
**本项目与 ClaudeMind 完全隔离**。

### 插件隔离

已在 `d:\TestScheduler\.claude\settings.local.json` 中禁用 `agent-tools@deepbank-marketplace` 插件，该插件提供的所有 ClaudeMind 全局 Agent（`info-fetcher`、`collect-requirement-materials`、`api-auto-testing`、`executor` 等）在 TestScheduler 中不可用。
...
> 违反隔离规则会导致数据写入 `D:\ClaudeMind\memory\` 而非 `d:\TestScheduler\memory\`，造成项目隔离失效。
...
- ❌ 读取 `D:\ClaudeMind\AGENTS.md` 或任何 ClaudeMind 配置
